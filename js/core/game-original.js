...
let snakes = [];
let snakeAdapter = null; // ECS snake adapter (initialized when feature flag is enabled)
...
const pixelPlanets = [];
// Planet generation removed for performance optimization
...
const aiSkins = Object.keys(skinMetadata).filter(skin => skinMetadata[skin]);
...
window.addEventListener('touchstart', (e) => {
    ...
    if (currentTrack && currentTrack.paused && !musicMuted && gameStarted) {
...
window.submitToLeaderboard = async function () {
    ...
    try {
        ...
        if (result !== null && result !== undefined) {
            ...
            // Handle new response format with both daily and monthly ranks
            if (typeof result === 'object' && result.monthly_rank !== undefined) {
                rank = result.daily_rank || 'Submitted';
                monthlyRank = result.monthly_rank;
            } else {
            ...
            statusElement.innerHTML =
                `<div style="background: linear-gradient(135deg, rgba(78, 205, 196, 0.3) 0%, rgba(78, 205, 196, 0.1) 100%);
                            ...
                            <div style="color: #4ecdc4; font-size: 18px; margin-top: 8px;">
                                ...
                                ${typeof monthlyRank === 'number' ? `<br>📊 Monthly Rank: #${monthlyRank}` : ''}
                            </div>
                        </div>`;
        ...
        if (window.leaderboardModule && window.leaderboardModule.submitScore) {
            window.leaderboardModule.submitScore(
                ...
                if (typeof result === 'object' && result.monthly_rank !== undefined) {
                    ...
                    // Old response format
                    dailyRank = result;
                    // Record as monthly rank for backward compatibility
                    if (window.playerStats && typeof result === 'number') {
                        window.playerStats.recordLeaderboardRank(result, true);
                    }
                }
                markScoreSubmitted();
                isSubmitting = false;
...
function drawBackground() {
    ...
    */
    ...
    // Planet rendering removed for performance optimization

    // Draw floating space stations
    drawSpaceStations();
...
function startGame() {
    ...
    // Initialize ECS snake adapter if feature flag is enabled
    if (window.featureFlags && window.featureFlags.isEnabled('useECSSnakes')) {
        if (window.GameSnakeAdapter && window.snakeIntegration) {
            snakeAdapter = new window.GameSnakeAdapter(window.snakeIntegration);
            gameLogger.debug('SNAKE', 'ECS snake adapter initialized');
        } else {
            gameLogger.warn('SNAKE', 'Feature flag enabled but ECS components not loaded, using legacy snakes');
        }
    }
    ...
    // Create player snake
    if (snakeAdapter) {
        // Use ECS snake system
        playerSnake = snakeAdapter.createPlayerSnake(WORLD_SIZE / 2, WORLD_SIZE / 2);
        gameLogger.debug('SNAKE', 'Player snake created via ECS adapter');
    } else {
        // Use legacy Snake class
    ...
    if (gameMode !== 'cozy') {
        for (let i = 0; i < MAX_AI_SNAKES; i++) {
            ...
            if (snakeAdapter) {
                // Use ECS snake system
                aiSnake = snakeAdapter.createAISnake(x, y);
                gameLogger.debug('SNAKE', `AI snake ${i + 1} created via ECS adapter`);
            } else {
                // Use legacy Snake class
                aiSnake = new Snake(x, y, false);
            }
...
function stopGame() {
    ...
    if (snakeAdapter) {
        snakeAdapter.reset();
        gameLogger.debug('SNAKE', 'ECS snake adapter reset');
    }
    snakes = [];
    playerSnake = null;
    window.playerSnake = null;
...
function gameLoop(currentTime) {
    ...
    while (accumulator >= FIXED_TIMESTEP && updates < MAX_UPDATES) {
        ...
        // Update all snakes via ECS or legacy
        if (snakeAdapter) {
            // Use ECS snake system for all snake updates
            snakeAdapter.updateAll(1.0);
        } else {
            // Use legacy snake update loop
...
if ((gameMode === 'classic' || gameMode === 'infinite') && playerSnake && playerSnake.alive) {
}
...
if (gameMode !== 'cozy') {
    ...
    aiRespawnQueue = aiRespawnQueue.filter(respawnTime => {
        if (currentTime >= respawnTime && aiSnakes < MAX_AI_SNAKES) {
            ...
            if (snakeAdapter) {
                // Use ECS snake system
                newAISnake = snakeAdapter.createAISnake(x, y);
                gameLogger.debug('SNAKE', 'AI snake respawned via ECS adapter');
            } else {
                // Use legacy Snake class
                newAISnake = new Snake(x, y, false);
            }
...
if (playerSnake && playerRespawnTimer <= 0 && (!deathSequenceActive || isRespawning) && !playerSnake.alive && !waitingForRespawnInput) {
    ...
    if (snakeAdapter) {
        // Use ECS snake system
        playerSnake = snakeAdapter.createPlayerSnake(WORLD_SIZE / 2, WORLD_SIZE / 2);
        gameLogger.debug('SNAKE', 'Player snake respawned via ECS adapter');
    } else {
        // Use legacy Snake class
        playerSnake = new Snake(WORLD_SIZE / 2, WORLD_SIZE / 2, true);
    }
