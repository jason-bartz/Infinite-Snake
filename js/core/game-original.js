        ...
        window.forceRespawn = function() {
            ...
            console.log('[RESPAWN] Reset leaderboardSubmitted flag and timestamp in forceRespawn');
        }
        
        // Handle permadeath in Classic mode
        function handlePermadeath() {
            console.log('[PERMADEATH] Game over in Classic mode');
        ...
        window.handleReviveOrRespawn = function() {
            ...
            if (deathSequenceActive) {
                console.log('[RESPAWN] Forcing end of death sequence');
            ...
            leaderboardSubmitted = false;
            lastSubmissionTime = 0;
            console.log('[RESPAWN] Reset leaderboardSubmitted flag and timestamp for new game');
        }
        ...
        function updateUI() {
            ...
            if (!playerSnake.alive && playerRespawnTimer > 0 && !deathSequenceActive) {
                ...
                console.log('[DEATH] Player is dead, respawn timer:', respawnSeconds, 'seconds');
                
                // Check if this is the final death in classic mode (4th death)
                if (gameMode === 'classic' && deathCount >= 4 && revivesRemaining === 0) {
                    console.log('[DEATH] Final death in Classic mode - skip death screen, go to game over');
                ...
                if (respawnOverlay) {
                    ...
                    if (!window.deathLeaderboardChecked) {
                        ...
                        console.log('[LEADERBOARD] ⚡ AUTO-SUBMISSION CHECK ⚡');
                        ...
                        if (playTime >= 5 && canSubmitScore()) {
                            ...
                            if (window.leaderboardModule && window.leaderboardModule.submitScore) {
                                ...
                                window.leaderboardModule.submitScore(
                                    ...
                                    console.log('[LEADERBOARD] Result type:', typeof result);
                                    
                                    // submitScore returns the rank as a number or 'Submitted' if rank is null
                                    if (result !== null && result !== undefined) {
                                        if (typeof result === 'number') {
                                            console.log('[LEADERBOARD] ✅ Score submitted successfully! Daily rank:', result);
                                            const globalRankEl = document.getElementById('globalRank');
                                            if (globalRankEl) {
                                                globalRankEl.textContent = `#${result}`;
                                                console.log('[LEADERBOARD] Updated rank display to:', result);
                                            }
                                        } else if (result === 'Submitted') {
                                            console.log('[LEADERBOARD] ✅ Score submitted successfully! (No rank available)');
                                            const globalRankEl = document.getElementById('globalRank');
                                            if (globalRankEl) {
                                                globalRankEl.textContent = 'Submitted';
                                                console.log('[LEADERBOARD] Updated rank display to: Submitted');
                                            }
                                        }
                                        
                                        // Already marked as submitted above
                                        console.log('[LEADERBOARD] Submission completed successfully');
                                    } else {
                                    ...
                                    leaderboardSubmitted = false;
                                    console.log('[LEADERBOARD] Reset flag, will retry on next update cycle');
                                });
                            } else {
                                    ...
                                    initLeaderboard().then(() => {
                                        if (window.leaderboardModule && window.leaderboardModule.submitScore) {
                                            ...
                                            window.leaderboardModule.submitScore(
                                                ...
                                                console.log('[LEADERBOARD] Retry submission result:', result);
                                                if (result && typeof result === 'number') {
                                                    console.log('[LEADERBOARD] ✅ Retry successful! Daily rank:', result);
                                                    const globalRankEl = document.getElementById('globalRank');
                                                    if (globalRankEl) {
                                                        globalRankEl.textContent = `#${result}`;
                                                    }
        ...
        function gameLoop(currentTime) {
            ...
            if (playerSnake && (playerSnake.isDying || !playerSnake.alive) && !isRespawning) {
                ...
                if (!deathSequenceActive && (playerSnake.isDying || !playerSnake.alive) && playerRespawnTimer <= 0 && !deathProcessed && !isRespawning) {
                    console.log('[DEATH SEQUENCE] Starting death animation');
                    ...
                    if (gameMode === 'classic' && !window.deathCountIncremented) {
                        ...
                        console.log('[DEATH SYSTEM] State:', {
                            deathCount,
                            revivesRemaining,
                            deathSequenceActive,
                            deathProcessed,
                    ...
                    console.log('[DEATH SEQUENCE] Camera animation initialized:', {
                ...
                if (deathSequenceActive && !deathProcessed) {
                    ...
                    if (deathSequenceTimer >= 200 && deathSequenceTimer <= 1200) {
                        ...
                        console.log('[DEATH SEQUENCE] Camera zoom progress:', {
                    ...
                    if (deathSequenceTimer >= DEATH_SEQUENCE_DURATION && !deathProcessed) {
                        console.log('[DEATH SEQUENCE] Death sequence complete, starting respawn timer');
                        deathSequenceActive = false;
                        deathCameraAnimation.active = false;
                        ...
                        if (gameMode === 'classic' && deathCount >= 4 && revivesRemaining === 0) {
                            console.log('[DEATH SEQUENCE] Final death in Classic mode - skip respawn timer');
                        ...
                        console.log('🎮💀 [DEATH] Player died, attempting auto-submit...');
                    console.log('🎮💀 [DEATH] Current state:', {
                        ...
                        if (window.leaderboardModule && window.leaderboardModule.submitScore) {
                            ...
                            try {
                                ...
                                playerSnake.kills
                                    ...
                                    console.log('[DEATH] Score submission result:', result);
                                    console.log('[DEATH] Result type:', typeof result);
                                    console.log('[DEATH] Result value:', result);
                                    
                                    // submitScore returns the rank as a number or 'Submitted' if rank is null
                                    if (result !== null && result !== undefined) {
                                        console.log('[DEATH] Valid result received:', result);
                                        
                                        // Update rank display immediately
                                        const rankEl = document.getElementById('globalRank');
                                        console.log('[DEATH] Found rank element:', !!rankEl);
                                        
                                        if (rankEl) {
                                            if (typeof result === 'number') {
                                                rankEl.textContent = `#${result}`;
                                                console.log('[DEATH] ✅ Updated rank display to:', `#${result}`);
                                            } else if (result === 'Submitted') {
                                                rankEl.textContent = 'Submitted';
                                                console.log('[DEATH] ✅ Score submitted (no rank available)');
                                            }
                                            
                                            // Double-check the update
                                            console.log('[DEATH] Rank element now shows:', rankEl.textContent);
                                        } else {
                                            ...
                                            console.log('[DEATH] Found elements with "rank" in ID:', allElements.length);
                                            allElements.forEach(el => console.log('[DEATH] -', el.id, el.textContent));
                                        }
                                        
                                        // Already set to true above
                                        console.log('[DEATH] Submission completed successfully');
                                    } else {
                            ...
                            const retryInterval = setInterval(() => {
                                if (window.leaderboardModule && window.leaderboardModule.submitScore && canSubmitScore()) {
                                    ...
                                    window.leaderboardModule.submitScore(
                                        ...
                                        console.log('[DEATH] Score submitted on retry!', result);
                                        if (result !== null && result !== undefined) {
                                            const rankEl = document.getElementById('globalRank');
                                            if (rankEl) {
                                                if (typeof result === 'number') {
                                                    rankEl.textContent = `#${result}`;
                                                    console.log('[DEATH] Retry: Updated rank display to:', `#${result}`);
                                                } else if (result === 'Submitted') {
                                                    rankEl.textContent = 'Submitted';
                                                    console.log('[DEATH] Retry: Score submitted (no rank available)');
                                                }
                                            }
                                            // Already set to true above
            ...
            if (playerRespawnTimer > 0) {
                ...
                if (playerRespawnTimer < 0) {
                    console.log('[RESPAWN] Timer is negative:', playerRespawnTimer);
                }
